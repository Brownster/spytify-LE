...
def track_events(
    ...
    """Subscribe to MPRIS track change events from Spotify desktop.
    ...
    ``player_name`` selects the MPRIS service (usually ``spotify`` for Spotify desktop).

    Uses signal-based detection for efficient real-time track monitoring.
    """
    bus = SessionBus()
    ...
    def handler(_iface, changed, _invalid):
        ...
        if md:
            ...
            track = TrackInfo(
                ...
                year=None,  # Will be fetched from LastFM
                genre=None,  # Will be fetched from LastFM
            )
    ...
    # Use signal-based track detection for Spotify desktop
